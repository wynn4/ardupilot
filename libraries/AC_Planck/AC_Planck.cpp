...
bool AC_Planck::get_accel_cmd(float &accel_n,
  float &accel_e,
  float &yaw_cd,
  float &vz_cms,
  bool &is_yaw_rate)
{
  if((AP_HAL::millis() - _accel_cmd.timestamp_ms) >= _cmd_timeout_accel_ms)
    return false;
  
  accel_n = _accel_cmd.accel_n;
  accel_e = _accel_cmd.accel_e;
  yaw_cd = _accel_cmd.yaw_cd;
