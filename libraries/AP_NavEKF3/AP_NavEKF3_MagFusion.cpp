...
void NavEKF3_core::SelectMagFusion()
{
    ...
    if (magcal == MagCal::EXTERNAL_YAW || magcal == MagCal::EXTERNAL_YAW_FALLBACK) {
        ...
        if (now - last_gps_yaw_fusion_ms < 5000) {
            // don't fallback to magnetometer fusion for 5s
            return;
        }
...
bool NavEKF3_core::learnMagBiasFromGPS(void)
{
    ...
    stateStruct.body_magfield -= err * frontend->_mag_learn_rate;
    ...
    if (err_length > frontend->_mag_learn_thresh) {
